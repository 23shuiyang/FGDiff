...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.ca_attention == 1:
            ...
            q = self.q(x).reshape(B, -1, self.sa_num_heads, C // self.sa_num_heads).permute(0, 2, 1, 3)
            ...
            x = (attn @ v).transpose(1, 2).reshape(B, -1, C) + \
                self.local_conv(v.transpose(1, 2).reshape(B, -1, C).transpose(1, 2).view(B, C, H, W)).view(B, C,
                                                                                                           -1).transpose(
                    1, 2)
        x = self.proj(x)
        x = self.proj_drop(x)
...
class Head(nn.Module):
    def __init__(self, head_conv, dim, mask_chans=1):
        ...
        if mask_chans != 0:
            self.mask_proj = nn.Sequential(
                nn.Conv2d(mask_chans, dim, kernel_size=head_conv, stride=2, padding=3 if head_conv == 7 else 1,
                          bias=False),
...
class SMT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for k in range(0, len(embed_dims)):
            self.time_embed.append(nn.Sequential(
                nn.Linear(embed_dims[k], 4 * embed_dims[k]),
                nn.SiLU(),
                nn.Linear(4 * embed_dims[k], embed_dims[k]),
            ))
        ...
        for i in range(num_stages):
            if i == 0:
                patch_embed = Head(head_conv, embed_dims[i], mask_chans=self.mask_chans)  #
            else:
    ...
    def forward_features(self, cond_img, s=None):
        B = cond_img.shape[0]
        layer_features = []
    ...
    def forward(self, cond_img, sal_img=None):
        x = self.forward_features(cond_img, sal_img)
        # x = self.head(x)
        return x

    def forward_encode(self, cond_img, sal_img=None):
        return self.forward(cond_img, sal_img)
